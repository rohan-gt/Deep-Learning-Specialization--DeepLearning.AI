...
def get_angles_test(target):
    ...
    assert result.shape == (position, d_model), (
...
def scaled_dot_product_attention_test(target):
    ...
    assert tuple(tf.shape(weights).numpy()) == (q.shape[0], k.shape[1]), (
    ...
    assert tuple(tf.shape(attention).numpy()) == (q.shape[0], v.shape[1]), (
...
def EncoderLayer_test(target):
    ...
    assert tuple(tf.shape(encoded).numpy()) == (1, q.shape[1], q.shape[2]), (
...
def Decoder_test(target, create_look_ahead_mask, create_padding_mask):
    ...
    x, attention_weights = decoderk(
        x_array, encoderq_output, False, look_ahead_mask, None
    )
    ...
    assert type(attention_weights) == dict, (
    ...
    x, attention_weights = decoderk(
        x_array, encoderq_output, True, look_ahead_mask, create_padding_mask(x_array)
    )
